...
- `deploy/init/topics-init.sh`: Tạo các topic chuẩn cho hệ thống (`iam.events`, `document.events`, `file.events`, `tag.events`, `version.events`, `workflow.events`, `signature.events`, `ocr.events`, `ops.events`, `search.events`, `audit.events`, `retention.events`). Có thể override broker/partition thông qua các biến môi trường `KAFKA_BROKER`, `KAFKA_TOPIC_PARTITIONS`, `KAFKA_TOPIC_REPLICAS` khi cần.
