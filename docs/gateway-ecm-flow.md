...
- Các API viewer mới (`GET /api/viewer/{versionId}` + PdfRenderer `/api/viewer/pdf/{versionId}`, proxy `word`/`excel`) vẫn forward token người dùng để tải metadata/stream từ ECM, giúp frontend chỉ cần gọi Gateway và không phải tải file gốc trực tiếp.
