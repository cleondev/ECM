    ...
    ISystemClock clock)
{
    ...
    public async Task<OperationResult<UserSummaryResult>> HandleAsync(AssignUserRoleCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (role is null)
        {
            return OperationResult<UserSummaryResult>.Failure($"Role '{command.RoleId}' was not found.");
        }
