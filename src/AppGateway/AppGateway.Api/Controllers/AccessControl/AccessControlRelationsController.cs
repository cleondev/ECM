...
namespace AppGateway.Api.Controllers.AccessControl;

[ApiController]
[Route("api/gateway/access-control/relations")]
[Authorize]
public sealed class AccessControlRelationsController(IEcmApiClient client) : ControllerBase
{
    ...
    public async Task<IReadOnlyCollection<AccessRelationDto>> GetBySubjectAsync(Guid subjectId, CancellationToken cancellationToken)
        => await _client.GetRelationsBySubjectAsync(subjectId, cancellationToken);
    ...
    public async Task<IActionResult> PostAsync([FromBody] CreateAccessRelationRequestDto request, CancellationToken cancellationToken)
    {
        ...
        return CreatedAtAction(nameof(GetBySubjectAsync), new { subjectId = relation.SubjectId }, relation);
    }

    [HttpDelete("subjects/{subjectId:guid}/objects/{objectType}/{objectId:guid}")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> DeleteAsync(Guid subjectId, string objectType, Guid objectId, [FromQuery] string relation, CancellationToken cancellationToken)
    {
        var deleted = await _client.DeleteRelationAsync(subjectId, objectType, objectId, relation, cancellationToken);
