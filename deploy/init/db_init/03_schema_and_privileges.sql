...
CREATE SCHEMA IF NOT EXISTS webhook AUTHORIZATION ecm_owner;

-- Phân quyền
GRANT USAGE ON SCHEMA iam, doc, file, wf, search, ocr, ops, webhook TO ecm_app;
GRANT USAGE, CREATE ON SCHEMA iam, doc, file, wf, search, ocr, ops, webhook TO ecm_migrator;

-- Default privileges: cấp quyền cho object tạo mới
ALTER DEFAULT PRIVILEGES FOR ROLE ecm_owner IN SCHEMA iam, doc, file, wf, search, ocr, ops, webhook
...
ALTER DEFAULT PRIVILEGES FOR ROLE ecm_migrator IN SCHEMA iam, doc, file, wf, search, ocr, ops, webhook
  GRANT SELECT, INSERT, UPDATE, DELETE, REFERENCES ON TABLES TO ecm_app;
ALTER DEFAULT PRIVILEGES FOR ROLE ecm_migrator IN SCHEMA iam, doc, file, wf, search, ocr, ops, webhook
  GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO ecm_app;
ALTER DEFAULT PRIVILEGES FOR ROLE ecm_migrator IN SCHEMA iam, doc, file, wf, search, ocr, ops, webhook
...
GRANT SELECT, INSERT, UPDATE, DELETE, REFERENCES ON ALL TABLES    IN SCHEMA iam, doc, file, wf, search, ocr, ops, webhook TO ecm_app;
GRANT USAGE, SELECT, UPDATE                  ON ALL SEQUENCES IN SCHEMA iam, doc, file, wf, search, ocr, ops, webhook TO ecm_app;
GRANT EXECUTE                                ON ALL FUNCTIONS IN SCHEMA iam, doc, file, wf, search, ocr, ops, webhook TO ecm_app;
